...
- 作業ディレクトリ: `D:\AI\OBSStreaming\obs-guide-mini-launcher`
...
2. `npm run validate` を実行し、`dist/obs-guide-mini-launcher-representative-report.json` が生成されることを確認する。
3. レポート JSON を開き、happy-path、missing-required、warning、mixed-batch の summary が期待値通りであることを確認する。
4. OBS 運用メモとして host adapter の guide/action 一覧を確認する。
